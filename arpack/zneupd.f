...
c                 %------------------------------------------%
c 
c          Original code (causes segfault):                  
c                 workev(j) = zdotc(j, workl(ihbds), 1,
c    &                        workl(invsub+(j-1)*ldq), 1)
            call zdotcc(j,workl(ihbds),1,workl(invsub+(j-1)*ldq),1,
     &                  workev(j))
 40         continue
c
            if (msglvl .gt. 2) then
...
c     Wrapper for zdotc, using a subroutine instead of function
      subroutine zdotcc(n,zx,incx,zy,incy,outputt)
      double complex zx(*),zy(*), outputt
      integer i,incx,incy,ix,iy,n
      outputt = zdotc(n, zx, incx, zy, incy)
      end
