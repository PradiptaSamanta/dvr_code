...
!   write(iout,*) 'Total number of spatial orbital', nSpatialOrbs
  ...
  subroutine Calc1eOrbInts()
    ...
    allocate(OneEInt(nSpatialOrbs,nSpatialOrbs), stat=error)
    call allocerror(error)
...
!           write(76,*) ind_1, ind_2, int_value
    ...
    write(iout,'(X,a,f10.5)') 'Time taken for transformation = ', finish-start, 'seconds.'
